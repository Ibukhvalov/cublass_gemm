#include "kernel.hpp"
#include "utils.hpp"
#include "kernels/tf32_tensor_core_block128x64.cuh"
...
__device__ void store_fragment_with_bounds(
    ...
    store_matrix_sync(scratch, frag, kWmmaN, mem_row_major);
    for (int idx = threadIdx.x; idx < kWmmaM * kWmmaN; idx += kWarpSize) {
        ...
        if (global_row < m && global_col < n) {
            C[global_row * ldc + global_col] = scratch[local_row * kWmmaN + local_col];
        }
    }
}

__global__ void tf32_tensor_core_block128x64_kernel(
    ...
    store_fragment_with_bounds(C, n, block_row + warp_row_base + 0, block_col + warp_col_base + 0, m, n, acc[0][0], scratch);
    store_fragment_with_bounds(C, n, block_row + warp_row_base + 0, block_col + warp_col_base + kWmmaN, m, n, acc[0][1], scratch);
    store_fragment_with_bounds(C, n, block_row + warp_row_base + kWmmaM, block_col + warp_col_base + 0, m, n, acc[1][0], scratch);
    store_fragment_with_bounds(C, n, block_row + warp_row_base + kWmmaM, block_col + warp_col_base + kWmmaN, m, n, acc[1][1], scratch);
}
}

void Tf32TensorCoreBlock128x64Kernel::launch(tf32_block128x64_fp_t* dA, tf32_block128x64_fp_t* dB, tf32_block128x64_fp_t* dC, int m, int n, int k) {
    dim3 block_size(kWarpSize, kWarpsPerBlock);
    dim3 grid_size(::ceil_div(n, kBlockN), ::ceil_div(m, kBlockM));
    tf32_tensor_core_block128x64_kernel<<<grid_size, block_size>>>(dA, dB, dC, m, n, k);
}
