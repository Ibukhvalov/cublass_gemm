#include "kernel.hpp"
#include "utils.hpp"
#include "kernels/tf32_tensor_core_direct_store.cuh"
...
__device__ void store_fragment_with_bounds(
    float* C,
    int ldc,
    int base_row,
    int base_col,
    int m,
    int n,
    fragment<accumulator, kWmmaM, kWmmaN, kWmmaK, float>& frag,
    float* scratch,
    int scratch_ld) {
    if (base_row + kWmmaM <= m && base_col + kWmmaN <= n) {
        store_matrix_sync(C + base_row * ldc + base_col, frag, ldc, mem_row_major);
        return;
    }

    store_matrix_sync(scratch, frag, scratch_ld, mem_row_major);
    for (int row = threadIdx.x; row < kWmmaM * kWmmaN; row += kWarpSize) {
        const int local_row = row / kWmmaN;
        const int local_col = row % kWmmaN;
        const int global_row = base_row + local_row;
        const int global_col = base_col + local_col;
        if (global_row < m && global_col < n) {
            C[global_row * ldc + global_col] = scratch[local_row * scratch_ld + local_col];
        }
    }
}

__global__ void tf32_tensor_core_direct_store_kernel(
    ...
    __shared__ float boundary_store[kWarpsPerBlock][kWmmaM * kWmmaN];
    ...
    float* scratch = &boundary_store[warp_idx][0];
    store_fragment_with_bounds(C, n, block_row + warp_row_base + 0, block_col + warp_col_base + 0, m, n, acc[0][0], scratch, kWmmaN);
    store_fragment_with_bounds(C, n, block_row + warp_row_base + 0, block_col + warp_col_base + kWmmaN, m, n, acc[0][1], scratch, kWmmaN);
    store_fragment_with_bounds(C, n, block_row + warp_row_base + kWmmaM, block_col + warp_col_base + 0, m, n, acc[1][0], scratch, kWmmaN);
    store_fragment_with_bounds(C, n, block_row + warp_row_base + kWmmaM, block_col + warp_col_base + kWmmaN, m, n, acc[1][1], scratch, kWmmaN);
}
}

void Tf32TensorCoreDirectStoreKernel::launch(tf32_direct_store_fp_t* dA, tf32_direct_store_fp_t* dB, tf32_direct_store_fp_t* dC, int m, int n, int k) {
    dim3 block_size(kWarpSize, kWarpsPerBlock);
    dim3 grid_size(::ceil_div(n, kBlockN), ::ceil_div(m, kBlockM));
    tf32_tensor_core_direct_store_kernel<<<grid_size, block_size>>>(dA, dB, dC, m, n, k);
}
