...
#include "macro.hpp"
#include "utils.hpp"
#include "kernels/tf32_tensor_core_ptx_async_aligned.cuh"
...
__device__ inline void issue_aligned_async_copy_stage(
    float* smem,
    const float* A,
    const float* B,
    int n,
    ...
    for (int vec_idx = linear_tid; vec_idx < kBlockM * kAVecCols; vec_idx += kThreadsPerBlock) {
        ...
        __pipeline_memcpy_async(
            a_stage_ptr(smem, stage, row, col),
    ...
    for (int vec_idx = linear_tid; vec_idx < kBlockK * kBVecCols; vec_idx += kThreadsPerBlock) {
        ...
        __pipeline_memcpy_async(
            b_stage_ptr(smem, stage, row, col),
...
__global__ void tf32_tensor_core_ptx_async_aligned_kernel(
    ...
    int stage = 0;
    issue_aligned_async_copy_stage(smem, A, B, n, k, block_row, block_col, 0, stage, linear_tid);
    __pipeline_commit();
    ...
    for (int k0 = 0; k0 < k; k0 += kBlockK) {
        ...
        if (next_k0 < k) {
            issue_aligned_async_copy_stage(smem, A, B, n, k, block_row, block_col, next_k0, next_stage, linear_tid);
            __pipeline_commit();
        }
    ...
    for (int mi = 0; mi < 2; ++mi) {
        ...
        for (int ni = 0; ni < 4; ++ni) {
            ...
            C[row0 * n + col0] = acc[mi][ni][0];
            C[row0 * n + col1] = acc[mi][ni][1];
            C[row1 * n + col0] = acc[mi][ni][2];
            C[row1 * n + col1] = acc[mi][ni][3];
        }
    }
}
}

void Tf32TensorCorePtxAsyncAlignedKernel::launch(tf32_ptx_async_aligned_fp_t* dA, tf32_ptx_async_aligned_fp_t* dB, tf32_ptx_async_aligned_fp_t* dC, int m, int n, int k) {
    CHECK_CUDA(cudaFuncSetAttribute(
        tf32_tensor_core_ptx_async_aligned_kernel,
    ...
    tf32_tensor_core_ptx_async_aligned_kernel<<<grid_size, block_size, kSharedBytes>>>(dA, dB, dC, m, n, k);
}
