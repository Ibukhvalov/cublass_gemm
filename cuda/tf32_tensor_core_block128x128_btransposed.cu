#include "kernel.hpp"
#include "utils.hpp"
#include "kernels/tf32_tensor_core_block128x128_btransposed.cuh"
...
constexpr int kSharedStrideB = kBlockK + kSkew;
...
__global__ void tf32_tensor_core_block128x128_btransposed_kernel(
    ...
    __shared__ float BsCol[kBlockN][kSharedStrideB];
    ...
    for (int k0 = 0; k0 < k; k0 += kBlockK) {
        ...
        for (int idx = linear_tid; idx < kBlockK * kBlockN; idx += kWarpSize * kWarpsPerBlock) {
            ...
            BsCol[col][row] = __float_to_tf32(value);
        }
        ...
        for (int kk = 0; kk < kBlockK; kk += kWmmaK) {
            ...
            fragment<matrix_b, kWmmaM, kWmmaN, kWmmaK, precision::tf32, col_major> b_frag[2];
            ...
            load_matrix_sync(b_frag[0], &BsCol[warp_col_base + 0][kk], kSharedStrideB);
            load_matrix_sync(b_frag[1], &BsCol[warp_col_base + kWmmaN][kk], kSharedStrideB);
...
void Tf32TensorCoreBlock128x128BTransposedKernel::launch(tf32_block128x128_btransposed_fp_t* dA, tf32_block128x128_btransposed_fp_t* dB, tf32_block128x128_btransposed_fp_t* dC, int m, int n, int k) {
    dim3 block_size(kWarpSize, kWarpsPerBlock);
    dim3 grid_size(::ceil_div(n, kBlockN), ::ceil_div(m, kBlockM));
    tf32_tensor_core_block128x128_btransposed_kernel<<<grid_size, block_size>>>(dA, dB, dC, m, n, k);
}
