...
#include "macro.hpp"
#include "utils.hpp"
#include "kernels/tf32_tensor_core_ptx_async_warp32x64_stage3.cuh"
...
__device__ inline int advance_stage(int stage) {
    ++stage;
    return stage == kStages ? 0 : stage;
}

__device__ inline int stage_plus_two(int stage) {
    stage += 2;
    return stage >= kStages ? stage - kStages : stage;
}
...
__global__ void tf32_tensor_core_ptx_async_warp32x64_stage3_kernel(
    ...
    int stage = 0;
    issue_async_copy_stage(smem, A, B, m, n, k, block_row, block_col, 0, 0, linear_tid);
    __pipeline_commit();
    const bool has_second_stage = kBlockK < k;
    if (has_second_stage) {
        issue_async_copy_stage(smem, A, B, m, n, k, block_row, block_col, kBlockK, 1, linear_tid);
        __pipeline_commit();
    ...
    for (int k0 = 0; k0 < k; k0 += kBlockK) {
        ...
        const int prefetch_k0 = k0 + 2 * kBlockK;
        bool issued_prefetch = false;
        if (prefetch_k0 < k) {
            issue_async_copy_stage(smem, A, B, m, n, k, block_row, block_col, prefetch_k0, stage_plus_two(stage), linear_tid);
            __pipeline_commit();
...
void Tf32TensorCorePtxAsyncWarp32x64Stage3Kernel::launch(tf32_ptx_async_warp32x64_stage3_fp_t* dA, tf32_ptx_async_warp32x64_stage3_fp_t* dB, tf32_ptx_async_warp32x64_stage3_fp_t* dC, int m, int n, int k) {
    CHECK_CUDA(cudaFuncSetAttribute(
        tf32_tensor_core_ptx_async_warp32x64_stage3_kernel,
    ...
    tf32_tensor_core_ptx_async_warp32x64_stage3_kernel<<<grid_size, block_size, kSharedBytes>>>(dA, dB, dC, m, n, k);
}
