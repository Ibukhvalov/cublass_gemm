#include "kernel.hpp"
#include "utils.hpp"
#include "kernels/tf32_tensor_core_block128x128_skewed.cuh"
...
__global__ void tf32_tensor_core_block128x128_skewed_kernel(
...
void Tf32TensorCoreBlock128x128SkewedKernel::launch(tf32_block128x128_skewed_fp_t* dA, tf32_block128x128_skewed_fp_t* dB, tf32_block128x128_skewed_fp_t* dC, int m, int n, int k) {
    dim3 block_size(kWarpSize, kWarpsPerBlock);
    dim3 grid_size(::ceil_div(n, kBlockN), ::ceil_div(m, kBlockM));
    tf32_tensor_core_block128x128_skewed_kernel<<<grid_size, block_size>>>(dA, dB, dC, m, n, k);
}
