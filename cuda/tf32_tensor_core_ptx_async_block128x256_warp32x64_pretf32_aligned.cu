...
#include "macro.hpp"
#include "utils.hpp"
#include "kernels/tf32_tensor_core_ptx_async_block128x256_warp32x64_pretf32_aligned.cuh"
...
__device__ inline void convert_stage_to_tf32(float* smem, int stage, int linear_tid) {
    float* a_base = smem + stage * kAStageElems;
    for (int idx = linear_tid; idx < kAStageElems; idx += kThreadsPerBlock) {
        a_base[idx] = nvcuda::wmma::__float_to_tf32(a_base[idx]);
    }

    float* b_base = smem + kStages * kAStageElems + stage * kBStageElems;
    for (int idx = linear_tid; idx < kBStageElems; idx += kThreadsPerBlock) {
        b_base[idx] = nvcuda::wmma::__float_to_tf32(b_base[idx]);
    }
}
...
__device__ inline void load_a_frag(
    ...
    a[0] = __float_as_uint(*a_stage_ptr(smem, stage, row_base + group_id, kk + thread_id_in_group));
    a[1] = __float_as_uint(*a_stage_ptr(smem, stage, row_base + group_id + 8, kk + thread_id_in_group));
    a[2] = __float_as_uint(*a_stage_ptr(smem, stage, row_base + group_id, kk + thread_id_in_group + 4));
    a[3] = __float_as_uint(*a_stage_ptr(smem, stage, row_base + group_id + 8, kk + thread_id_in_group + 4));
}
...
__device__ inline void load_b_frag(
    ...
    int group_id,
    int thread_id_in_group) {
    b[0] = __float_as_uint(*b_stage_ptr(smem, stage, kk + thread_id_in_group, col_base + group_id));
    b[1] = __float_as_uint(*b_stage_ptr(smem, stage, kk + thread_id_in_group + 4, col_base + group_id));
}

__global__ void tf32_tensor_core_ptx_async_block128x256_warp32x64_pretf32_aligned_kernel(
    ...
    __syncthreads();
    convert_stage_to_tf32(smem, stage, linear_tid);
    __syncthreads();
...
void Tf32TensorCorePtxAsyncBlock128x256Warp32x64Pretf32AlignedKernel::launch(tf32_ptx_async_block128x256_warp32x64_pretf32_aligned_fp_t* dA, tf32_ptx_async_block128x256_warp32x64_pretf32_aligned_fp_t* dB, tf32_ptx_async_block128x256_warp32x64_pretf32_aligned_fp_t* dC, int m, int n, int k) {
    CHECK_CUDA(cudaFuncSetAttribute(
        tf32_tensor_core_ptx_async_block128x256_warp32x64_pretf32_aligned_kernel,
    ...
    tf32_tensor_core_ptx_async_block128x256_warp32x64_pretf32_aligned_kernel<<<grid_size, block_size, kSharedBytes>>>(dA, dB, dC, m, n, k);
}
