...
#include "macro.hpp"
#include "utils.hpp"
#include "kernels/tf32_tensor_core_ptx_async_block128x256_warp32x64_btranspose_aligned.cuh"
...
__device__ inline float* b_stage_ptr(float* smem, int stage, int col, int row) {
    float* b_base = smem + kStages * kAStageElems;
    return b_base + stage * kBStageElems + col * kBTransposedStride + row;
}

__device__ inline void issue_copy_stage(
    float* smem,
    const float* A,
    const float* B,
    int n,
    ...
    for (int vec_idx = linear_tid; vec_idx < kBlockK * kBVecCols; vec_idx += kThreadsPerBlock) {
        ...
        const float4 values = *reinterpret_cast<const float4*>(&B[(k0 + row) * n + block_col + col]);
        *b_stage_ptr(smem, stage, col + 0, row) = values.x;
        *b_stage_ptr(smem, stage, col + 1, row) = values.y;
        *b_stage_ptr(smem, stage, col + 2, row) = values.z;
        *b_stage_ptr(smem, stage, col + 3, row) = values.w;
    }
}
...
__device__ inline void load_b_frag(
    ...
    int group_id,
    int thread_id_in_group) {
    const float b0 = nvcuda::wmma::__float_to_tf32(*b_stage_ptr(smem, stage, col_base + group_id, kk + thread_id_in_group));
    const float b1 = nvcuda::wmma::__float_to_tf32(*b_stage_ptr(smem, stage, col_base + group_id, kk + thread_id_in_group + 4));
    b[0] = __float_as_uint(b0);
    b[1] = __float_as_uint(b1);
}

__global__ void tf32_tensor_core_ptx_async_block128x256_warp32x64_btranspose_aligned_kernel(
    ...
    int stage = 0;
    issue_copy_stage(smem, A, B, n, k, block_row, block_col, 0, stage, linear_tid);
    __pipeline_commit();
    ...
    for (int k0 = 0; k0 < k; k0 += kBlockK) {
        ...
        if (next_k0 < k) {
            issue_copy_stage(smem, A, B, n, k, block_row, block_col, next_k0, next_stage, linear_tid);
            __pipeline_commit();
        }
...
void Tf32TensorCorePtxAsyncBlock128x256Warp32x64BtransposeAlignedKernel::launch(tf32_ptx_async_block128x256_warp32x64_btranspose_aligned_fp_t* dA, tf32_ptx_async_block128x256_warp32x64_btranspose_aligned_fp_t* dB, tf32_ptx_async_block128x256_warp32x64_btranspose_aligned_fp_t* dC, int m, int n, int k) {
    CHECK_CUDA(cudaFuncSetAttribute(
        tf32_tensor_core_ptx_async_block128x256_warp32x64_btranspose_aligned_kernel,
    ...
    tf32_tensor_core_ptx_async_block128x256_warp32x64_btranspose_aligned_kernel<<<grid_size, block_size, kSharedBytes>>>(dA, dB, dC, m, n, k);
}
